...
const ProductCardHeader = ({productName,productDescription}:{productName:string; productDescription: string;})=>{
    const {cardText} = styles;
    return(
        <>
            <h6 className="card-text" style={cardText}>{productName}.</h6>
            <p className="card-text" style={cardText}>{productDescription}.</p>
        </>
    )
}
...
const ProductCardFooter = ()=>{
    ...
    const leftSideButtons = ()=>{
        return ['View',"Edit"].map(btn=>(
            <button className="btn btn-sm primary-btn" type='button' key={btn}>
                {btn}
            </button>
        ))
    }
